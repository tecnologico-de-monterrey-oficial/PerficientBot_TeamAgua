...
export class SendTokensComponent implements OnInit {
  ...
  enviarTokens(): void {
    if (this.outlookToken && this.githubToken && this.azureToken) {
      ...
      this.http
        .post(' https://perficient-bot-service-dannyjr08.cloud.okteto.net:8000/api/DatabasePOSTTokens', datosTokens)
        .subscribe(
          (response: any) => {
  ...
  getTokens(): void {
    ...
    this.http
      .get(` https://perficient-bot-service-dannyjr08.cloud.okteto.net:8000/api/DatabaseGETTokens/${sub}`)
      .subscribe(
        (response: any) => {
