...
export class HrSearchComponent implements OnInit{
  ...
  getImage(persona: any): void {
    ...
    this.http.get(` https://perficient-bot-service-backend-flask-dannyjr08.cloud.okteto.net/CV/${subFixed}`).subscribe(
      (response: any) => {
  ...
  getSummary(persona: any): void {
    ...
    // then use the fixed sub in your HTTP requests
    this.http.get(` https://perficient-bot-service-backend-flask-dannyjr08.cloud.okteto.net/GPTtext/${subFixed}`).subscribe(
      (response: any) => {
